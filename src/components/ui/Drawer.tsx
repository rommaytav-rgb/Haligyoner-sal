...
export function Drawer({
  ...
  return (
    <div className="fixed inset-0 z-50" role="dialog" aria-modal="true">
      <button aria-label="Close" onClick={onClose} className="absolute inset-0 bg-ink/25 animate-fade-in" />
      <aside
        className={cn(
          ...
          "sm:inset-y-0 sm:left-auto sm:right-0 sm:max-h-none sm:w-[420px] sm:rounded-none sm:rounded-l-3xl sm:border-l sm:border-t-0",
        )}
      >
        <header className="sticky top-0 flex items-center justify-between border-b border-line bg-white/95 px-5 py-4 backdrop-blur">
          ...
          <button onClick={onClose} className="rounded-lg p-1.5 text-ink-mute hover:bg-paper-sunk hover:text-ink" aria-label="Close">
