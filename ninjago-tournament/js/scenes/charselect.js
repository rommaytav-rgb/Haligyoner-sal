...
NT.Scenes.CharSelect = class CharSelect {
  ...
  slots(W, H) {
    ...
    const cx = W / 2; const sp = land ? Math.min(W * 0.15, base * 0.27) : W * 0.19; const pr = Math.min(0.066 * base, sp * 0.42); const top = land ? 0.1 * H : 0.09 * H; const rowH = land ? 0.19 * H : pr * 2.3;
  ...
  layout(W, H) {
    ...
    const ay = this.slotPos[6].y; const ax = land ? Math.max(br + 10, this.slotPos[3].x - br * 2.2) : br + 12;
    ...
    const infoTop = land ? 0.56 * H : this.slotPos[12].y + this.slotPos[12].r + 20;
