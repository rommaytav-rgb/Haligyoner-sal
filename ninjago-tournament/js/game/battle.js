...
NT.Scenes.Battle = class Battle {
  ...
  setPhase(p) {
    ...
    if (p === 'victory') {
      ...
      this.addBanner('vs', 'VS', cdef.name, 2.4); this.waveText = this.plan.boss ? 'ARENA BOSS' : 'CHAMPION';
      this.cam.tzoom = 1.25;
  ...
  renderBanners(ctx, W, H) {
    ...
    for (const b of this.banners) {
      ...
      if (b.style === 'round') {
        ...
        if (this.plan.boss) UI.goldText(ctx, 'ARENA BOSS', W / 2, y + pr * 1.75, base * 0.03, { mid: '#ff5a5a', top: '#ffb0b0', bot: '#7a0a0a' });
